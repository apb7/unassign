...
module.exports = class Unassign {
  ...
  async getNoResponseLabel (type) {
    ...
    var timestamp = this.since(days).toISOString().replace(/\.\d{3}\w$/, '')
    ...
    this.logger.info('SEARCH RESULTS FOR LABELLING', searchResults.data)
  ...
  async getUnassign (type) {
    ...
    const params = {owner, repo, state:'open', assignee:'*', labels:[noResponseLabel], sort:'updated', direction:'desc', since:timestamp, per_page:maxActionsPerRun}
  ...
  async mark (type, issue) {
    ...
    var perform = Boolean(process.env.PERFORM)

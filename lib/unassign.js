...
module.exports = class Unassign {
  ...
  async getNoResponseLabel (type) {
    ...
    function isInactive(issue) {
      this.logger.info('ISSUE IN INACTIVE', issue)
      var updatedAt = new Date(issue.updated_at)
      var inactivityTime = new Date(timestamp)
      this.logger.info('UPDATED AT', updatedAt)
      this.logger.info('INACTIVE TIME', inactivityTime)
      if(updatedAt <= inactivityTime) {
        this.logger.info('INSIDE NO RESPONSE IF BLOCK')
        return issue
      }
    }
    ...
    this.logger.info('PARAMS TO MARK ISSUES', params)
    const searchResults = await this.github.issues.getForRepo(params)
    this.logger.info('SEARCH RESULTS FOR UNASSIGNMENT', searchResults.data)
    const filteredSearchResults = searchResults.data.filter(isInactive)
    this.logger.info('SEARCH RESULTS FOR UNASSIGNMENT', searchResults.data)
    this.logger.info('FILTERED SEARCH RESULTS FOR UNASSIGNMENT', filteredSearchResults)
  ...
  async getUnassign (type) {
    ...
    function isReallyInactive(issue) {
      this.logger.info('ISSUE IN REALLY INACTIVE', issue)
      var updatedAt = new Date(issue.updated_at)
      var reallyInactivityTime = new Date(timestamp)
      this.logger.info('UPDATED AT', updatedAt)
      this.logger.info('REALLY INACTIVE TIME', reallyInactivityTime)
      if(updatedAt <= reallyInactivityTime) {
        this.logger.info('INSIDE UNASSIGN IF BLOCK')
        return issue
      }
    }
    ...
    this.logger.info('PARAMS TO UNASSIGN ISSUES', params)
    const searchResults = await this.github.issues.getForRepo(params)
    this.logger.info('SEARCH RESULTS FOR UNASSIGNMENT', searchResults.data)

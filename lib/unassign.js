...
module.exports = class Unassign {
  ...
  search (type, days, query) {
    ...
    // This query probably will not work since we cannot search for assigned issues globally.
    // The only query that works is that for a particular repo.
    // Ex. is:open updated:<${timestamp} ${query} assignee:*
    // would work.
    query = `repo:${owner}/${repo} is:open updated:<${timestamp} ${query} assignee:*`
    ...
    searchResults = this.github.search.issues(params)
  ...
  async mark (type, issue) {
    ...
    const noResponseLabel = 'issue assignee: no-response'
    var markComment = 'Hi @author, this issue has been marked for no response.' // Change message later on.
    const number = issue.number
    const issueAuthor = issue.user.login
    var customMarkComment = markComment.replace('@author', '@' + issueAuthor)
    if (perform) {
